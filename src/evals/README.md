...
|------|------------|--------|
| `feasibility` | Plan + Edit | Daily duration ≤ window (default 720 min), per-leg travel ≤ 90 min, daily travel ≤ 240 min, activity count consistent with pace |
...
| `interest_coverage` | Plan only (Review runtime) | Requested interests appear when enforceable POIs exist in registry |
...
| `overbudget_day.json` | Fails feasibility (840 min scheduled vs 720 min budget) |
